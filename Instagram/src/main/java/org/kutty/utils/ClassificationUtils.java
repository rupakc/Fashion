...
public class ClassificationUtils {
	...
	public static List<Benchmark> getBenchmarkAdaptor(List<Post> postList,String type) { 
		...
		for (Post post : postList)  { 
			...
			if (post.getTagset() != null && !post.getTagset().isEmpty()) {  
				
				benchmark.setTagSet(post.getTagset());
			} 
			...
			else if(type.equalsIgnoreCase(Constants.GIVEAWAY_TYPE)) { 
				
				if(post.getGiveawayLabel() != null && !post.getGiveawayLabel().equalsIgnoreCase("null")) {
					
					benchmark.setActualLabel(post.getGiveawayLabel());
					resultList.add(benchmark);
				}
			}
