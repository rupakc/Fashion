...
public class NaiveBayesGiveaway { 
	...
	public String classifyGiveaway(String captionText,String tagset) { 
		...
		for (int i = 1; i <= Constants.MAX_MODEL_NUM; i++) { 
			...
			multiplyWithPriors(caption_probability);
	...
	/** 
	 * Given the array of posterior probabilities multiplies them with the posterior probabilities
	 * @param probabilityArray Double array containing the posterior probability values
	 */
	public void multiplyWithPriors(double probabilityArray[]) { 
		
		Map<String,Double> spamPriorMap;
		String filename = Constants.GIVEAWAY_FOLDER + "/" + Constants.GIVEAWAY_PRIOR_FILE; 
