...
public class ModelWeight {
	...
	public ModelWeight(int modelNumber,String channelName,String type) {  
		...
		else if(this.type.equalsIgnoreCase(Constants.GIVEAWAY_TYPE)) { 
			this.modelFilePath = this.channelName.toLowerCase().trim() + "/" + "giveaway_model_weights.txt";
		}
	}
	...
	public void calculateModelPrediction(List<Benchmark> resultList) { 
		...
		if (this.type.equalsIgnoreCase(Constants.SENTIMENT_TYPE)) { 
			...
			for (Benchmark benchmark : resultList) { 
				...
				if (!this.channelName.equalsIgnoreCase("Instagram")) { 
					
					predictedLabel = ClassificationUtils.sanitizeString(naive.classifySentimentOtherChannels(benchmark.getContent())); 
					
				} else {  
					
					predictedLabel = ClassificationUtils.sanitizeString(naive.classifySentimentInstagram(benchmark.getContent(), benchmark.getTagSet()));
				} 
			...
			for (Benchmark benchmark : resultList) { 
				...
				if(!this.channelName.equalsIgnoreCase("Instagram")) {
					...
					predictedLabel = ClassificationUtils.sanitizeString(naive.classifySpamInstagram(benchmark.getContent(), benchmark.getTagSet()));
				} 
			...
			NaiveBayesGiveaway naive = new NaiveBayesGiveaway(this.modelNumber); 
			
			for (Benchmark benchmark : resultList) { 
				
				predictedLabel = ClassificationUtils.sanitizeString(naive.classifyGiveaway(benchmark.getContent(),benchmark.getTagSet()));
				benchmark.setPredictedLabel(predictedLabel);
			}
		}
	}
	...
	public static void main(String args[]) { 
		...
		mw.modelWeightCalculationPipelineAllModels("twitter", "spam");
	}
}
