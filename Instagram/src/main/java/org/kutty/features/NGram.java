...
/** 
 * Extracts the NGram features from a given post
...
public class NGram {
	...
	/** 
	 * For Instagram selects the filename to write the unigram probabilities of the unigrams
	 * @param type String containing the type of classification being made 
	 * i.e. giveaway detection,spam detection etc
	 */ 
	...
	public void writeTagGramToFileSelector(String type) { 
		...
		if (type.equalsIgnoreCase("sentiment")) { 
			...
			writeGramToFile("twitter/positive_1.txt",positive_tag_map);
			writeGramToFile("twitter/negative_1.txt",negative_tag_map);
			writeGramToFile("twitter/neutral_1.txt",neutral_tag_map);
		}
	...
	public void writeGramToFileSelector(String type) { 
		...
		if (type.equalsIgnoreCase("giveaway")) { 
			...
			writeGramToFile("", real_giveaway_count_map);
			writeGramToFile("", fake_giveaway_count_map);
		}
		...
		while(index != -1) { 
			...
			if (index != -1 && !ngram_map.containsKey(ngram)) {
				if (this.NGRAM == 1) {
					...
					count = getNGramCountUtilForNGreaterThanOne(ngram, post_list, ngram_label, type);
				}
				ngram_map.put(ngram,count); 
			}
		}
	...
	/** 
	 * For Ngrams greater than one calculates the probability for that ngram
	 * @param ngram String containing the ngram
	 * @param post_list List<Post> containing the list of posts from which ngram counts have to be generated
	 * @param ngram_label String containing the class label of the ngram
	 * @param type String containing the type of analysis being made i.e. Sentiment Analysis, Spam detection etc
	 * @return
	 */
	public double getNGramCountUtilForNGreaterThanOne(String ngram,List<Post> post_list, String ngram_label,String type) { 

		double count = 0.0;
		String sentence = ""; 
		String post_label = ""; 
		String spam_label = ""; 
		double n_minusone_count = 0.0;
		String minusOneGram = getNMinusOneGram(ngram.trim());
		...
		for (Post temp : post_list) { 
			...
			if (spam_label != null && post_label != null && (!spam_label.isEmpty() || !post_label.isEmpty())) {
				...
				if (ngram_label != null && (ngram_label.equalsIgnoreCase(post_label) || ngram_label.equalsIgnoreCase(spam_label))) {
					...
					count = count + getSubStringCount(ngram, sentence);
					n_minusone_count = n_minusone_count + getSubStringCount(minusOneGram, sentence);
				}
			}
		...
		return ((count+1)/(n_minusone_count+1));
	}
	
	/** 
	 * Truncates the last word in a string and returns the rest for a multi-word string 
	 * @param ngram String containing the multi word ngram
	 * @return String containing the (n-1) gram
	 */
	public String getNMinusOneGram(String ngram) { 
		
		String minusOne = "";
		int index = -1;
		
		index = ngram.lastIndexOf(" ");
		
		if (index != -1) { 
			
			minusOne = ngram.substring(0, index);
			minusOne = minusOne.trim();
		}
	...
	public static void main(String args[]) {  
		...
		for (int i = 1; i <= 5; i++) {
			new NGram().NGramExtractionPipeline("facebook/split_5.txt", i, "Facebook", "sentiment");
		}
	}
}
